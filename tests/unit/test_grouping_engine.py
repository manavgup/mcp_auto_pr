...
class TestGroupingEngine:
    ...
    def grouping_engine(self):
        ...
        with (
            ...
            patch("mcp_pr_recommender.services.grouping_engine.settings") as mock_ge_settings,
        ):
    ...
    def sample_file_changes(self):
        ...
        return [FileStatus(path=path, status_code="M", lines_added=10, lines_deleted=2) for path in file_paths]
    ...
    def create_analysis_with_files(self, files):
        ...
        # Create working directory changes with the files
        working_directory = WorkingDirectoryChanges(
            modified_files=files,  # Put all files as modified for simplicity
            added_files=[],
            deleted_files=[],
            renamed_files=[],
            untracked_files=[],
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations(self, grouping_engine, sample_file_changes):
        ...
        with patch("mcp_pr_recommender.services.grouping_engine.settings") as mock_runtime_settings:
            mock_runtime_settings.return_value.enable_llm_analysis = False  # Skip LLM to avoid API key
            mock_runtime_settings.return_value.max_files_per_pr = 8
            mock_runtime_settings.return_value.similarity_threshold = 0.7

            result = await grouping_engine.generate_pr_recommendations(analysis, "semantic")
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_with_different_strategy(self, grouping_engine, sample_file_changes):
        ...
        with patch("mcp_pr_recommender.services.grouping_engine.settings") as mock_runtime_settings:
            ...
            result = await grouping_engine.generate_pr_recommendations(analysis, "logical")
    ...
    async def test_group_files_empty_input(self, grouping_engine):
        ...
        from mcp_local_repo_analyzer.models import (
            BranchStatus,
        ...
        # Create working directory changes with no files
        working_directory = WorkingDirectoryChanges(
            modified_files=[],
            added_files=[],
            deleted_files=[],
        ...
        # Create real analysis object with proper repository status
        analysis = OutstandingChangesAnalysis(
            repository_path="test_repo",
            analysis_timestamp=datetime.now(),
    ...
    @pytest.mark.asyncio
    async def test_group_files_by_directory_strategy(self, grouping_engine, sample_file_changes):
        ...
        with patch("mcp_pr_recommender.services.grouping_engine.settings") as mock_runtime_settings:
            ...
            result = await grouping_engine.generate_pr_recommendations(analysis, "directory")
    ...
    @pytest.mark.asyncio
    async def test_group_files_invalid_strategy(self, grouping_engine, sample_file_changes):
        ...
        with patch("mcp_pr_recommender.services.grouping_engine.settings") as mock_runtime_settings:
            ...
            result = await grouping_engine.generate_pr_recommendations(analysis, "nonexistent_strategy")
