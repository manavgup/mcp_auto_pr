...
# Removed factory imports - using simple test data instead
...
class TestPRRecommenderTool:
    ...
    def pr_recommender_tool(self):
        ...
        with (
            ...
            patch("mcp_pr_recommender.services.semantic_analyzer.settings") as mock_sa_settings,
            patch("mcp_pr_recommender.tools.pr_recommender_tool.SemanticAnalyzer") as mock_semantic_analyzer_class,
        ):
    ...
    def sample_file_changes(self):
        ...
        return [
            {"path": "src/auth/login.py", "status": "modified", "lines_changed": 15},
            {"path": "src/auth/logout.py", "status": "modified", "lines_changed": 8},
            {"path": "src/models/user.py", "status": "added", "lines_changed": 45},
            {"path": "tests/test_auth.py", "status": "modified", "lines_changed": 20},
            {"path": "tests/test_user.py", "status": "added", "lines_changed": 30},
            {"path": "config/settings.py", "status": "modified", "lines_changed": 5},
        ]
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_success(self, pr_recommender_tool, sample_file_changes):
        ...
        with patch.object(pr_recommender_tool, "_extract_all_files", return_value=mock_files):
            # Mock the semantic analyzer's response - make it async
            from mcp_pr_recommender.models.recommendations import PRRecommendation
    ...
    async def test_generate_pr_recommendations_empty_changes(self, pr_recommender_tool):
        ...
        with patch.object(pr_recommender_tool, "_extract_all_files", return_value=[]):
            result = await pr_recommender_tool.generate_recommendations(analysis_data=analysis_data)

        assert result["error"] == "No files to analyze"
        assert result["recommendations"] == []

    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_invalid_strategy(self, pr_recommender_tool, sample_file_changes):
        ...
        mock_files = [
            FileStatus(path="src/test.py", status_code="M", lines_added=10, lines_deleted=2),
        ]
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_large_changeset(self, pr_recommender_tool):
        ...
        mock_files = [
            FileStatus(path=f"src/file_{i}.py", status_code="M", lines_added=5, lines_deleted=2) for i in range(30)
        ]
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_high_risk_files(self, pr_recommender_tool):
        ...
        high_risk_recs = [r for r in result["recommendations"] if r["risk_level"] in ["high"]]
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_recommendations_error_handling(self, pr_recommender_tool, sample_file_changes):
        ...
        with patch.object(pr_recommender_tool, "_extract_all_files", return_value=mock_files):
            ...
            pr_recommender_tool.semantic_analyzer.analyze_and_generate_prs.side_effect = Exception("Analysis failed")
