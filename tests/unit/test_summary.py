...
from mcp_local_repo_analyzer.models.analysis_repository import (
    BranchStatus,
    RepositoryStatus,
)
from mcp_local_repo_analyzer.models.categorization import ChangeCategorization
from mcp_local_repo_analyzer.models.changes import (
    StagedChanges,
    WorkingDirectoryChanges,
)
from mcp_local_repo_analyzer.models.commits import StashedChanges, UnpushedCommit
...
class TestGetOutstandingSummary:
    ...
    async def test_get_outstanding_summary_with_changes(self):
        ...
        # Setup mocks
        self.mock_services["git_client"].get_branch_info.return_value = {"current_branch": "main"}
        ...
        mock_repo_status = RepositoryStatus(
            ...
            working_directory=WorkingDirectoryChanges(
                ...
                modified_files=[FileStatus(path="src/main.py", status_code="M", staged=False)],
                added_files=[],
                deleted_files=[],
                renamed_files=[],
                untracked_files=[],
            ...
            staged_changes=StagedChanges(
                staged_files=[FileStatus(path="tests/test.py", status_code="A", staged=True)],
        ...
        self.mock_services["status_tracker"].get_repository_status.return_value = mock_repo_status
        ...
        self.mock_services["diff_analyzer"].categorize_changes.return_value = mock_categories
    ...
    async def test_get_outstanding_summary_invalid_repo(self):
        ...
        with (
            ...
            patch(
                "mcp_local_repo_analyzer.tools.summary.find_git_root",
                return_value=None,
            ),
        ):
            result = await call_tool_helper(mcp, "get_outstanding_summary", repository_path="/invalid/path")
    ...
    async def test_get_outstanding_summary_high_risk(self):
        ...
        result = await call_tool_helper(mcp, "get_outstanding_summary", repository_path=self.repo_path)
...
class TestAnalyzeRepositoryHealth:
    ...
    async def test_analyze_repository_health_excellent(self):
        ...
        self.mock_services["status_tracker"].get_health_metrics.return_value = mock_health_metrics

        # Convert to use call_tool_helper - will replace the call pattern below"analyze_repository_health")
        AsyncMock()

        result = await call_tool_helper(mcp, "analyze_repository_health", repository_path=self.repo_path)
    ...
    async def test_analyze_repository_health_error(self):
        ...
        self.mock_services["status_tracker"].get_health_metrics.side_effect = Exception("Service failure")
...
class TestGetPushReadiness:
    ...
    async def test_get_push_readiness_ready(self):
        ...
        self.mock_services["git_client"].get_branch_info.return_value = {"current_branch": "feature/test"}
        ...
        result = await call_tool_helper(mcp, "get_push_readiness", repository_path=self.repo_path)
    ...
    async def test_get_push_readiness_blockers(self):
        ...
        mock_repo_status = RepositoryStatus(
            ...
            working_directory=WorkingDirectoryChanges(
                all_files=[FileStatus(path="src/main.py", status_code="M", staged=False)],
                modified_files=[FileStatus(path="src/main.py", status_code="M", staged=False)],
                added_files=[],
                deleted_files=[],
                renamed_files=[],
...
class TestAnalyzeStashedChanges:
    ...
    async def test_analyze_stashed_changes_with_stashes(self):
        ...
        self.mock_services["change_detector"].detect_stashed_changes.return_value = mock_stashes

        result = await call_tool_helper(mcp, "analyze_stashed_changes", repository_path=self.repo_path)
    ...
    async def test_analyze_stashed_changes_error(self):
        ...
        self.mock_services["change_detector"].detect_stashed_changes.side_effect = Exception("Git error")
...
class TestDetectConflicts:
    ...
    async def test_detect_conflicts_with_conflicts(self):
        ...
        mock_working_changes = WorkingDirectoryChanges(
            all_files=[
                ...
                FileStatus(path="config.json", status_code="M", lines_added=10, lines_deleted=5),
            ],
            modified_files=[],
            added_files=[],
            deleted_files=[],
        ...
        self.mock_services["change_detector"].detect_working_directory_changes = AsyncMock(
            return_value=mock_working_changes
        )
        self.mock_services["change_detector"].detect_staged_changes = AsyncMock(return_value=mock_staged_changes)
    ...
    async def test_detect_conflicts_no_conflicts(self):
        ...
        self.mock_services["git_client"].get_branch_info.return_value = {"current_branch": "feature/safe"}
        ...
        self.mock_services["change_detector"].detect_working_directory_changes.return_value = mock_working_changes
        self.mock_services["change_detector"].detect_staged_changes.return_value = mock_staged_changes
    ...
    async def test_detect_conflicts_error(self):
        ...
        self.mock_services["git_client"].get_branch_info.side_effect = Exception("Git error")
