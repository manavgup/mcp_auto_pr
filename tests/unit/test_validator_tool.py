...
def mock_settings():
    ...
    with patch("mcp_pr_recommender.tools.validator_tool.settings") as mock_settings_func:
...
class TestValidatorTool:
    ...
    @pytest.mark.asyncio
    async def test_validate_recommendations_valid(self, validator_tool, valid_recommendations):
        ...
        assert len(result["suggestions"]) >= 0  # May have suggestions even for valid recs

    @pytest.mark.asyncio
    async def test_validate_recommendations_invalid(self, validator_tool, invalid_recommendations):
    ...
    async def test_validate_file_overlap_detection(self, validator_tool):
        ...
        assert "file_overlaps" in conflict_analysis or "has_conflicts" in conflict_analysis
    ...
    async def test_validate_required_fields(self, validator_tool):
        ...
        assert any("files" in issue.lower() or "no files" in issue.lower() for issue in issues)
    ...
    @pytest.mark.asyncio
    async def test_quality_score_calculation(self, validator_tool, valid_recommendations):
